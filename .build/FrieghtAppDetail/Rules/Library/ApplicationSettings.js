...
export default class {
    ...
    static hasKey(context, key) {
     ...
     static setStringArray(context, key, value) {
        try {
            ...
            NativeScriptObject.getNativeScriptObject(context).appSettingsModule.setString(key, valueString);
        } catch (error) {x
            Logger.error('ApplicationSettings - setString', error);
        }
    }
    ...
    static getStringArray(context, key, defaultValue = '') {
        try {
            if (NativeScriptObject.getNativeScriptObject(context).appSettingsModule.hasKey(key)) {
                return JSON.parse(NativeScriptObject.getNativeScriptObject(context).appSettingsModule.getString(key, defaultValue));
            }
        } catch (error) {
            Logger.error('ApplicationSettings - getString', error);
        }
